...
class GCN(nn.Module):
    ...
    def fit_with_val(self, features, adj, labels, data, train_iters=200, report=False, initialize=True, verbose=False, normalize=True, patience=None, noval=False, **kwargs):
        ...
        if noval:
            ...
            self._train_with_val(labels, data, train_iters, verbose, report=self.report)

    def _train_with_val(self, labels, data, train_iters, verbose, adj_val=False, report=False):
        # print('************************************')
        # print(report)
        if adj_val:
        ...
        for i in range(train_iters):
            ...
            if report:
                lt = torch.LongTensor(data.labels_train).cuda()
                op = self.predict(data.feat_train, data.adj_train)
                act = utils.accuracy(op, lt)
                print("Train set results:",
                "accuracy= {:.4f}".format(act.item()))
